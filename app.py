...
def chain_result(pdf_d):
      ...
      prompt_template = """
        ...
        provided context specifically say, "answer is not available in the context", don't provide the wrong answer, be specific, write "REMOVE66" if  if the answer is not in
        provided context  \n\n
        Context:\n {context}?\n
        Question: \n{question}\n
        ...
        if uploaded_files:
            if "bool" in st.session_state:
                if st.session_state.bool==True:
                    ...
                    if "ujnjkn" in result1:#"does not provide any information" in result1 or "does not contain any information" in result1 or "answer is not available" in result1:
                          st.write("No answer") 
                    else:
                          st.write(result1)
